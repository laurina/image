...
#include <math.h>
 ...
 char fin_name[] = {"picture_0_salt&pepper.bmp"};
 char fout_name[] = {"picture_0_median_7.bmp"};
 unsigned char  *fin;
...
{	
	...
	for (i=halflenth; i<row-1; i++) {
		for (j=halflenth ; j<col-1; j++) {l = 0;
		     for (m = -halflenth; m <= halflenth; m++) {				//得邻域的像素值
